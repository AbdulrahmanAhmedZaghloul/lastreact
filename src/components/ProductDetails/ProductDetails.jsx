...
export default function ProductDetails() {
    ...
    if (isError) {
        console.log(isError);
        return null; // Optionally render an error message to the user
    }
                    ...
                    <div className='w-1/4 mx-auto'>
                        <Slider className='p-5' {...settings}>
                            {getid?.images.map((src, index) => (
                                <img key={index} className='w-full mx-auto' src={src} alt={getid?.title} />
                            ))}
                        </Slider>
                    </div>
                    ...
                    {onCategory.map((product) => (
                        <div key={product.id} className='lg:w-1/6 md:w-1/3 sm:w-1/2 w-full group/item transition-all overflow-hidden p-4'>
                            <div className='product overflow-hidden'>
                                ...
                                <button onClick={() => addProductCart(product.id)} className='bg-green-700 p-0 opacity-0 translate-y-40 group-hover/item:opacity-[1] group-hover/item:translate-y-0 transition-all w-10/12 my-3 mx-auto rounded-sm py-1 text-white'>Add to Cart</button>
                            </div>
                        </div>
                    ))}
                </div>
